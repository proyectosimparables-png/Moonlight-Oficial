...
interface ProductCardProps {
  ...
  nombre: string;
  precio: string | number; // Aceptamos ambos para evitar errores
}
...
const ProductCard = ({
  ...
  // 💰 Lógica de Precios (Versión robusta)
  ...
  return (
    <Card className="group overflow-hidden bg-white border border-[#ddd] hover:shadow-xl transition-all duration-300 rounded-lg">
      <CardContent className="p-0">
        ...
        {/* Contenedor de Imagen - Ahora con onClick para móviles */}
        <div 
        ...
        >
          ...
          {/* Overlay "Ver Detalle" (Visible mayormente en Desktop por el group-hover) */}
          ...
          {/* ❤️ Favoritos - Usamos e.stopPropagation para que no navegue al detalle al dar like */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              toggleFavorite(id);
          ...
          >
            <Star className={`h-5 w-5 ${isFavorite(id) ? "fill-[#f5c518] text-[#f5c518]" : "text-[#6c5b7b]"}`} />
          </button>
