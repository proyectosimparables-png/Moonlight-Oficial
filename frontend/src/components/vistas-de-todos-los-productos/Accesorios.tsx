...
const Accesorios = () => {
  ...
  const categoriaNombre = "Accesorios";
  ...
  useEffect(() => {
    const cargarCategoria = async () => {
      try {
        ...
        if (!categoria) {
          setError("Categoría Accesorios no encontrada");
          setCargando(false);
          return;
        }
  ...
  if (cargando) return <div>Cargando accesorios...</div>;
  if (error) return <div>{error}</div>;

  return (
    <div>
      <h1 className="text-center my-6 text-xl font-semibold">Accesorios</h1>

      {productos.length === 0 ? (
        <p className="text-center text-gray-500 my-10">
          Próximamente cargaremos los accesorios ✨
        </p>
      ) : (
