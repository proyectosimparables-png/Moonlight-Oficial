...
const BangtanHome = () => {
  ...
  const categoriaNombre = "Bangtan Home";
  ...
  useEffect(() => {
    const cargarCategoria = async () => {
      try {
        ...
        if (!categoria) {
          setError("Categoría Bangtan Home no encontrada");
          setCargando(false);
          return;
        }
  ...
  if (cargando) return <div>Cargando Bangtan Home...</div>;
  if (error) return <div>{error}</div>;

  return (
    <div>
      <h1 className="text-center my-6 text-xl font-semibold">Bangtan Home</h1>

      {productos.length === 0 ? (
        <p className="text-center text-gray-500 my-10">
          Próximamente cargaremos los productos de Bangtan Home 💜
        </p>
      ) : (
